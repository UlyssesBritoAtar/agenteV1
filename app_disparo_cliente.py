...
def cliente_disparar():
    try:
        ...
        intervalo = int(request.form['intervalo_mensagem'])
        quantidade_pausa = int(request.form['quantidade_pausa'])
        tempo_pausa = int(request.form['tempo_pausa'])
...
def executar_disparo(df, intervalo, quantidade_pausa, tempo_pausa, mensagem_modelo,
    ...
    with app.app_context():
        ...
        for i in range(len(df)):
            ...
            if (i + 1) % quantidade_pausa == 0:
                adicionar_log(cliente_id, f"⏸ Pausa de {tempo_pausa}s após {quantidade_pausa} envios.")
