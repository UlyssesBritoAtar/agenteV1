...
def cliente_pause():
    ...
    if cliente_id not in pause_status or not pause_status[cliente_id]["running"]:
        return jsonify({"status": "Já finalizado ou disparo não iniciado."}), 400

    status = get_pause_status(cliente_id)

    # Uso do lock para garantir que o estado seja alterado de forma segura
    with store_lock:
...
def cliente_cancelar():
    ...
    with store_lock:
        # Verificar se o cliente tem eventos de disparo antes de limpar
    ...
    # Atualizar o status do agente no banco de dados
    agente = Agente.query.filter_by(cliente_id=cliente_id).first()
    if agente:
        agente.disparo_ativo = False
        db.session.commit()

    # Adicionando o log do cancelamento
